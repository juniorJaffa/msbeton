...
import { LogOut, Users, Truck, Wrench, Layers, RefreshCw, ClipboardList, TrendingUp, MoreHorizontal, Search, Fingerprint, Server, Eye, ShieldCheck, Clock } from "lucide-react";
...
type Tab = "betony" | "sluzby" | "doprava" | "klienti" | "objednavky" | "historia" | "statistiky" | "seo" | "server";
...
const HistoriaTab    = lazy(() => import("./tabs/HistoriaTab"));
...
export default function AdminDashboard() {
  ...
  const [tab, setTab] = useState<Tab>(() => {
    ...
    const valid: Tab[] = ["betony", "sluzby", "doprava", "klienti", "objednavky", "historia", "statistiky", "seo", "server"];
  ...
  const [historiaFilter, setHistoriaFilter] = useState<{ sub: "zalohy" | "cashflow"; clientId?: string; date?: string } | undefined>(undefined);

  const handleGoToHistoria = (filter: { sub: "zalohy" | "cashflow"; clientId?: string; date?: string }) => {
          ...
          <div className="relative flex-1">
            ...
            {moreOpen && (
              <div className="absolute top-full right-0 mt-px w-44 bg-white border border-gray-200 rounded-sm shadow-xl overflow-hidden z-50">
                ...
                <button onClick={() => { setTab("seo"); window.location.hash = "seo"; setMoreOpen(false); }}
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-xs font-bold transition-colors border-t border-gray-100 ${tab === "seo" ? "text-primary bg-primary/5" : "text-gray-600 hover:bg-gray-50"}`}>
      ...
      <div id="admin-content" className="fixed top-[86px] sm:top-20 left-0 right-0 bottom-0 overflow-y-auto">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4 pb-8">
          <Suspense fallback={<TabSpinner />}>
            ...
            {tab === "historia" && <HistoriaTab key={`${historiaFilter?.sub}-${historiaFilter?.clientId}-${historiaFilter?.date}`} initialSub={historiaFilter?.sub} initialClientId={historiaFilter?.clientId} initialDate={historiaFilter?.date} onGoToClient={(loginId) => { setGoToClientId(loginId); setTab("klienti"); window.location.hash = "klienti"; }} onGoToOrder={(orderId) => { setGoToOrdersSearch(undefined); setGoToOrdersFocusId(orderId); setTab("objednavky"); window.location.hash = "objednavky"; }} />}
